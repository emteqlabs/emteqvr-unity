...
namespace EmteqLabs
{
    public static class Logger
    {
        ...
        private const string LogHeader = "[EmteqVRPlugin]";
        
        public static void LogMessage(string logMessage, LogType logType = LogType.Log)
        {
            if (EmteqVRManager.Instance.ShowLogMessages)
            {
        ...
        private static void WriteToLogFile(string logMessage, LogType logType)
        {
            if (_logWriter == null)
            {
                ...
                _logWriter = new StreamWriter(Path.Combine(Application.persistentDataPath, "EmteqVRPlugin.log"));
